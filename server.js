...
await pool.query(
  `INSERT INTO devices (email,paired_device) 
   VALUES ($1, $2)`,
  [email, pairingCode]
);
...
app.post('/set-angle', async (req, res) => {
  try {
    ...
    res.json({ 
      success: true,
      angle: updateResult.rows[0].angle 
    });
  } catch (error) {
    console.error('POST /set-angle error:', error);
    res.status(500).json({ error: 'Failed to update servo angle' });
  }
});
...
const result = await pool.query(
  `UPDATE devices
  SET device_name = $1 WHERE paired_device= $2  AND email = $3   RETURNING *`,
...
app.get('/get-mode', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT mode FROM devices WHERE paired_device = $1',
      [pairingCode]
    );
